...
export class Archiver {
  ...
  public async flush(): Promise<number> {
    ...
    // entry per (platform, stream_id), which is the most recently collected.
    // stream_id alone is not unique now that it is only unique within a
    // platform.
    const dedupe = <T extends { platform: Platform; stream_id: string }>(
      rows: T[]
    ): T[] => {
      const byId = new Map<string, T>();
      for (const r of rows) byId.set(`${r.platform}\u0000${r.stream_id}`, r);
      return [...byId.values()];
    };
    ...
    try {
      ...
      for (let i = 0; i < index.length; i += INSERT_BATCH) {
        ...
        // A stream can legitimately reach the archiver twice (one that
        ...
        insert.text += ` ON CONFLICT (platform,stream_id) DO UPDATE SET
      ...
      // A single buffer can hold more than one platform, so this is grouped.
      let archivedCount = 0;
      let deletedCount = 0;
      for (const [platform, ids] of archivedIds) {
        archivedCount += ids.length;
        const deleted = await client.query<{ stream_id: string }>(
          'DELETE FROM stream WHERE platform = $2 AND stream_id = ANY($1::text[]) AND ended_at IS NOT NULL RETURNING stream_id',
          [ids, platform]
        );
      ...
      if (deletedCount !== archivedCount) {
        this.log.warn(
          { archived: archivedCount, deleted: deletedCount },
